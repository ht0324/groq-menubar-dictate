...
/// emits one structured line via `log(to:)` when diagnostics are enabled.
struct WorkflowTiming {
    ...
    /// Emits a single structured diagnostics line. Optional fields fall back to
    /// `-1` (or `"unknown"`) so every run logs the same set of keys.
    func log(to logger: Logger) {
        ...
        logger.info(
            "Workflow timing result=\(result, privacy: .public) audio_bytes=\(audioFileSizeBytes, privacy: .public) recording_s=\(recordingDurationSeconds, format: .fixed(precision: 3)) upload_body_bytes=\(uploadBodySizeBytes, privacy: .public) total_ms=\(totalMilliseconds, format: .fixed(precision: 1)) stop_ms=\(stopRecordingMilliseconds, format: .fixed(precision: 1)) prep_ms=\(promptPreparationMilliseconds, format: .fixed(precision: 1)) transcribe_ms=\(transcriptionMilliseconds, format: .fixed(precision: 1)) upload_prep_ms=\(uploadPreparationMilliseconds, format: .fixed(precision: 1)) network_ms=\(networkRoundTripMilliseconds, format: .fixed(precision: 1)) task_interval_ms=\(taskIntervalMilliseconds, format: .fixed(precision: 1)) fetch_to_response_end_ms=\(fetchToResponseEndMilliseconds, format: .fixed(precision: 1)) dns_ms=\(domainLookupMilliseconds, format: .fixed(precision: 1)) tcp_ms=\(tcpConnectionMilliseconds, format: .fixed(precision: 1)) tls_ms=\(tlsHandshakeMilliseconds, format: .fixed(precision: 1)) request_upload_ms=\(requestUploadMilliseconds, format: .fixed(precision: 1)) first_byte_wait_ms=\(timeToFirstByteAfterUploadMilliseconds, format: .fixed(precision: 1)) response_download_ms=\(responseDownloadMilliseconds, format: .fixed(precision: 1)) reused_connection=\(reusedConnection, privacy: .public) network_protocol=\(networkProtocolName, privacy: .public) parse_ms=\(responseParseMilliseconds, format: .fixed(precision: 1)) post_ms=\(postProcessingMilliseconds, format: .fixed(precision: 1)) clipboard_ms=\(clipboardMilliseconds, format: .fixed(precision: 1)) paste_ms=\(pasteMilliseconds, format: .fixed(precision: 1))"
        )
    }
}
